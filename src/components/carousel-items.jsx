const CarouselItems = ({ imageLocation, imageSrc, subText, text, mobileImageSrc }) => {
  return (
    ...
    mb-[180px] flex lg:flex-row flex-col-reverse lg:w-[800px] lg:w-[300px] pb-[100px] lg:pb-[0px] mx-auto  justify-between border-[5px] border-solid
  border-[rgba(255,114,0,0.20)] p-8 lg:py-[9.5625rem] lg:px-[3.75rem] min-h-[44.375rem] lg:mb-[16.375rem]`}>      <CarouselImage imageLocation={imageLocation} imageSrc={imageSrc} mobileImageSrc={mobileImageSrc} />
...
const CarouselImage = ({ imageLocation, imageSrc, mobileImageSrc }) => {
  return (
    <div className={`-mb-[400px]  mt-10  ${getCorrectPosition(imageLocation)}`}>
      <img className={`${imageLocation == 'right' ? "w-[350px]" : "w-[370px]"}  h-full  lg:h-[45.625rem] lg:max-w-[38rem]`} src={mobileImageSrc} />
    </div>
  );
};
