const CarouselItems = ({ imageLocation, imageSrc, subText, text }) => {
  return (
    ...
    mt-10 flex xl:w-[800px] lg:w-[300px]   pb-[100px] mx-auto  justify-between border-[5px] border-solid
  border-[rgba(255,114,0,0.20)] p-8 xl:py-[9.5625rem] xl:px-[3.75rem] min-h-[44.375rem] lg:mb-[16.375rem]  `}
    >
      <CarouselImage imageLocation={imageLocation} imageSrc={imageSrc} />
      <div
        className={`${imageLocation == 'left' ? ' lg:pl-[20rem] ' : ''} flex w-full flex-col`}
      >
        <div
...
const getCorrectPosition = (imageLocation) => {
  if (imageLocation == 'right') {
    return `absolute lg:-right-[20rem] lg:top-[3.9375rem] -bottom-[300px]`;
  }
  if (imageLocation == 'left') {
    return `absolute lg:-left-[15rem] lg:top-[3.9375rem] -left-[10px]  -bottom-[300px]`;
  }
};
const CarouselImage = ({ imageLocation, imageSrc }) => {
  return (
    <div className={getCorrectPosition(imageLocation)}>
      <img className={` ${imageLocation == 'right' ? "w-[330px]" : ""}  h-full  xl:h-[45.625rem] xl:max-w-[38rem] `} src={imageSrc} />
    </div>
  );
};
